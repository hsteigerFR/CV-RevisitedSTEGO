...
if __name__ == "__main__":
    ...
    if params["dataset_create_step"]:
        ...
        print("1 - Creation of dataset through image slices")
        f = os.system(
            f"python ./src/create_dataset.py -i {params['raw_folder_path']} -o ./{params['project_name']}/imgs/train -output_s {params['slice_win_size']} -padding_s {params['slice_pad_step']}")
        if f:
            sys.exit(-1)

        print("2 - Output dataset sent to PyTorch folder")
        shutil.move(
    ...
    if params["ortho_eval_step"]:
        ...
        if params["class_mode"] == "manual":
            ...
            print("2 - Orthophoto slicing")
            f = os.system(
                f"python ./src/eval_ortho1.py -i_ortho {params['ortho_folder_path']} -o ./temp/part1 -output_s {params['ortho_win_size']} -padding_s {params['ortho_pad_step']}")
            if f:
                sys.exit(-1)
