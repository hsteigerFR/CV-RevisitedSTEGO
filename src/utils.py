...
def resize(classes: torch.Tensor, size: int):
    return F.interpolate(classes, (size, size),
...
def load_model(model_type, data_dir):
    if model_type == "robust_resnet50":
        ...
        model_weights_modified = {
            name.split('model.')[1]: value for name,
            value in model_weights['model'].items() if 'model' in name}
...
class UnsupervisedMetrics(Metric):
    ...
    def update(self, preds: torch.Tensor, target: torch.Tensor):
        with torch.no_grad():
            ...
            actual = actual[mask]
            preds = preds[mask]
            self.stats += torch.bincount((self.n_classes +
                                          self.extra_clusters) *
                                         actual +
                                         preds, minlength=self.n_classes *
                                         (self.n_classes +
                                          self.extra_clusters)) .reshape(self.n_classes, self.n_classes +
                                                                         self.extra_clusters).t().to(self.stats.device)
    ...
    def map_clusters(self, clusters):
        if self.extra_clusters == 0:
            ...
            missing = sorted(list(
                set(range(self.n_classes + self.extra_clusters)) - set(self.assignments[0])))
    ...
    def compute(self):
        ...
        metric_dict = {self.prefix +
                       "mIoU": iou[~torch.isnan(iou)].mean().item(), self.prefix +
                       "Accuracy": opc.item()}
...
def flexible_collate(batch):
    ...
    if isinstance(elem, torch.Tensor):
        ...
        return elem_type(*(flexible_collate(samples)
