...
class ContrastiveSegDataset(Dataset):
    def __init__(self,
        ...
        if pos_labels or pos_images:
            if not os.path.exists(feature_cache_file) or compute_knns:
                raise ValueError(
                    "could not find nn file {} please run precompute_knns".format(feature_cache_file))
            else:
    ...
    def __getitem__(self, ind):
        ...
        if self.pos_images or self.pos_labels:
            ind_pos = self.nns[ind][torch.randint(
                low=1, high=self.num_neighbors + 1, size=[]).item()]
