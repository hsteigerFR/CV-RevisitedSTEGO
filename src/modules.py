    ...
    def forward(self, img, n=1, return_class_feat=False):
        ...
        with torch.no_grad():
            ...
            if return_class_feat:
                return feat[:, :1, :].reshape(
                    feat.shape[0], 1, 1, -1).permute(0, 3, 1, 2)
...
class ClusterLookup(nn.Module):
    ...
    def forward(self, x, alpha, log_probs=False):
        ...
        if alpha is None:
            cluster_probs = F.one_hot(
                torch.argmax(
                    inner_products,
                    dim=1),
                self.clusters.shape[0]) .permute(
                0,
                3,
                1,
                2).to(
...
class FeaturePyramidNet(nn.Module):
    ...
    def _helper(x):
        ...
        return F.interpolate(x, 56, mode="bilinear",
...
class ContrastiveCorrelationLoss(nn.Module):
    ...
    def helper(self, f1, f2, c1, c2, shift):
        with torch.no_grad():
            # Comes straight from backbone which is currently frozen. this
            # saves mem.
...
class ContrastiveCRFLoss(nn.Module):
    ...
    def forward(self, guidance, clusters):
        ...
        sim_kernel = self.w1 * torch.exp(- coord_diff / (2 * self.alpha) - guidance_diff / (
            2 * self.beta)) + self.w2 * torch.exp(- coord_diff / (2 * self.gamma)) - self.shift
