...
if __name__ == "__main__":
    ...
    if params["dataset_create_step"]:
        ...
        print("2 - PCA model training for dimensionality reduction")
        f = os.system(
            f"python ./src/create_dataset2.py -i ./temp/part1 -o ./{params['project_name']}/pca_model.sav")
        if f:
            sys.exit(-1)

        print("3 - PCA model evaluation on dataset")
        f = os.system(
            f"python ./src/create_dataset3.py -i_imgs ./temp/part1 -i_pca ./{params['project_name']}/pca_model.sav -o ./{params['project_name']}/imgs/train -output_s {params['slice_win_size']} -n -padding_s {params['slice_pad_step']}")
        if f:
            sys.exit(-1)
        ...
        print("5 - Output dataset sent to PyTorch folder")
        shutil.move(
    ...
    if params["ortho_eval_step"]:
        ...
        pca_model_path = os.path.join(
            ...
            params["project_name"],
            "pca_model.sav")
        pca_norm_params_path = os.path.join(
            "C:/workspace/datadrive/pytorch-data/",
            params["project_name"],
            "norm_params.sav")
        ...
        if params["class_mode"] == "manual":
            ...
            print("2 - Orthophoto slicing and processing with PCA")
            f = os.system(
                f"python ./src/eval_ortho1.py -i_ortho {params['ortho_folder_path']} -i_pca {pca_model_path} -i_norm {pca_norm_params_path} -o ./temp/part1 -output_s {params['ortho_win_size']} -n -padding_s {params['ortho_pad_step']}")
            if f:
                sys.exit(-1)
            ...
            print("1 - Orthophoto slicing and processing with PCA")
            f = os.system(
                f"python ./src/eval_ortho1.py -i_ortho {params['ortho_folder_path']} -i_pca {pca_model_path} -i_norm {pca_norm_params_path} -o ./temp/part1 -output_s {params['ortho_win_size']} -n -padding_s {params['ortho_pad_step']}")
            if f:
                sys.exit(-1)
